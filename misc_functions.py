...
def normalize(values, min_range=-1, max_range=1):
    small = min(values)
    large = max(values)
    if large > small:
        return (max_range - min_range) * ((values - small) / float(large - small)) + min_range
    else:
        return (max_range - min_range) * ((values - small) / 0.000001) + min_range

def normalizeBACKUP(x, minimum, maximum, start_range=-1, end_range=1):
    percent = (x - minimum) / (maximum - minimum)
    return (end_range - start_range) * percent + start_range
